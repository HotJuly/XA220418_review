...
- 2XX: 表示成功处理请求
  ...
  - 201->当前请求成功,但是参数不对
- 3XX: 需要重定向, 浏览器直接跳转
  - 302->地址重定向,用户请求A链接,服务器通过返回302,引导用户跳转到指定的B链接
  - 304->协商缓存,当用户二次请求某个缓存好的资源之后,服务器会引导浏览器找到内存中对应的文件,进行复用
- 4XX: 客户端请求错误
  - 403->服务器有该资源,但是权限不够,拒绝访问
    - 确定好自己的帐号密码没问题,就找后端
  - 404->找不到该资源,没有该资源,该接口不存在
    - 问题:大概率是前端路径书写错误,小概率是后端没有该接口
- 5XX: 服务器端错误
  - 500->服务器崩溃,无法处理当前的请求代码
  - 501->服务器本次请求失败,因为请求方式不对

#### 区别ajax请求与一般HTTP请求

ajax是一种发送HTTP请求的技术,不是一种特殊的请求
...
1. 配置通用的基础路径和超时时间
   ...
   2. **超时时间可以延伸到请求并发数量**
      1. **小程序的请求最大并发数量->10个**
         1. **小程序的默认和最大超时时间为60s**
      2. **Chrome和Firefox浏览器的请求最大并发数量->6个**
         1. **网页开发的最大超时时间为5或者20s**
2. 显示请求进度条
   1. 显示进度条: 请求拦截器回调
   2. 结束进度条: 响应拦截器回调
...
- render 渲染：将颜色/文字/图片等渲染上去
  -  重绘:重新渲染,重新绘制
- 问题:重绘是否一定伴随着重排?
  - 如果只是改变颜色等信息,那么只会有重绘不会出现重排
  - 如果是改变了一个元素的宽高大小,位置等信息,那么重绘重排都会出现
  - 小总结:页面上只要想看到最新的效果,一定会重绘
- 扩展:重绘是将当前整个图层全部重新绘制一遍
  - 解决方案:开启新图层,最简单的开启方法(will-change:true)
  - 链接:https://blog.csdn.net/weixin_43294560/article/details/124902267
