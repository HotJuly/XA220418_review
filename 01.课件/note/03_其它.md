...
- 2XX: 表示成功处理请求
  ...
  - 201->当前请求成功,但是参数不对
- 3XX: 需要重定向, 浏览器直接跳转
  - 302->我们请求服务器的A链接,但是服务器引导我们跳转到B链接,也就是地址重定向
  - 304->是协商缓存,当我们二次请求某个资源的时候,服务器发现上次请求过该资源,而且该资源还存在于浏览器中,那么服务器就不会返回数据,而且重定向到浏览器中对应的文件地址上(也就是让浏览器复用上次的请求结果)
- 4XX: 客户端请求错误
  - 403->有该资源,但是由于权限原因不给你
    - 先用postman测试,如果还是403,直接找后端
  - 404->没有该资源,资源路径不存在
    - 如果开发中出现该报错,先检查自己的请求地址
    - 其次检查自己的proxy规则书写,以及是否有重启项目
- 5XX: 服务器端错误
  - 501->服务器无法处理当前请求,服务器代码运行出问题了

#### 区别ajax请求与一般HTTP请求

ajax只是浏览器发送请求的一种技术,不是一种特殊的请求,发送出去的实际上还是HTTP请求
...
1. 配置通用的基础路径和超时时间
   1. 为什么要配置基础路径->可以延伸到跨域相关
   2. 超时时间可以延伸到请求并发数上面
      1. Chrome和FireFox的请求并发数最多为6个
         1. 扩展:小程序最大并发数量是10个,由于通道数量比浏览器更多,所以超时时间默认60s
      2. 超时时间就是防止某个请求请求时间过长,导致请求通道被占用,延长后续请求的时间
         1. 请求超时,状态会直接变为404,视为失败,同时浏览器会自动发送下一个请求
2. 显示请求进度条
   1. 显示进度条: 请求拦截器回调
   2. 结束进度条: 响应拦截器回调
...
1. DNS 解析(想要访问一个服务器,就必须知道他的IP地址)
   1. 域名出现的目的:为了方便推广,方便记忆
      1. 劣化性能,减慢了展示项目的速度(尽量减少项目中请求的域名个数)
   2. DNS解析的目的就是将域名变成IP地址
...
- layout 布局(排版)：计算元素的位置和大小信息
  - 重排:当某些内容的大小等信息发生了变化,影响到了页面的布局,需要重新排版
- render 渲染：将颜色/文字/图片等渲染上去
  - 重绘:当某些内容的颜色等信息发生了变化,想要展示最新的结果,就需要重新渲染
  - 重绘不一定伴随这重排,但是重排一定伴随着重绘
