...
*/
  ...
  */
  ...
  /*
    问题1:请问Vue1中,DOM更新是同步更新还是异步更新?
      在Vue1中,更新DOM是同步更新的(整个流程里面没有出现异步任务)
      无论是Vue1还是Vue2或者Vue3,他们都是依靠GUI线程更新页面,所以渲染页面一定是异步的

    
    问题2:请问Vue2中,DOM更新是同步更新还是异步更新?
      在Vue2中,更新DOM是异步更新
        其实源码中,会将更新DOM的方法传递给nextTick进行执行,
          而nextTick中用到.then去开启我任务

    问题3:请问Vue1中,数据更新导致DOM更新的范围是多大(项目,组件,节点)
      在Vue1中,会找到指定的节点进行更新(精准更新)

    问题4:请问Vue2中,数据更新导致DOM更新的范围是多大(项目,组件,节点)
      在Vue2中,更新DOM的范围是整个组件(范围更新)
        感觉上去Vue1的更新方案似乎更好更精准,但是这个精准是需要付出代价的
          实际上Vue2的更新方案会更加节省内存,但是可能会存在误杀的情况
            误杀这个情况不需要担心,因为Vue2引入了diff算法,目的就是为了解决误杀的情况

    问题5:请问Vue1中,通过数组的下标操作数据,是否具有响应式效果?
      在Vue1的源码中,并没有对对象和数组进行区分操作,所以是具有响应式效果的
        因为数组也是对象,只是属性名都是0,1,2,3这种下标

    问题6:请问Vue2中,通过数组的下标操作数据,是否具有响应式效果?
      在Vue2中,不能直接通过数组的下标操作数据,否则没有响应式效果

      问题:为什么不对数组的下标进行数据劫持?
      回答:
        这其实是尤大大故意为之,
        因为我们平常开发很少直接通过下标去操作数组的内容,但是如果对数组的下标进行数据劫持操作,
          会浪费非常多的内容,去存储对应的dep对象

      问题:如何操作数组的数据才能具有响应式效果?
      回答:
        可以通过数组重写过的7种方法去操作数组,就会具有响应式的效果
        7种:push,pop,shift,unshift,splice,sort,reverse

      问题:Vue2是如何做到,不数据劫持数组的下标的?
        Vue2源码中,会判断当前数据劫持的数据是数组还是对象
          如果是数组,那么就使用for循环跳过对其下标的数据劫持,改为只劫持它内部的数据
            扩展:如果该数组内部的内容未来也不会修改,
              那么可以使用Object.freeze方法,将内部所有属性都进行冻结,防止生成无用的dep对象

      问题:Vue2是如何做到只对状态数据中的数组的7种方法进行重写的?
        Vue2会使用Object.create创建一个对象,该对象的隐式原型会指向Array的原型对象
          Vue2会在这个新对象身上,创建跟数组中对应的7种方法
        在做数据劫持的时候,如果遇到了数组,会将该数组的__proto__属性修改为刚才创建的那个对象
  */
